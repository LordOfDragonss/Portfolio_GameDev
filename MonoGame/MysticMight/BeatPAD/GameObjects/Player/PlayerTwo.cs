...
namespace BeatPAD
{
    public class PlayerTwo : PlayerParent
    {
        ...
        public PlayerTwo(PlayingState playstate) : base(playstate, AssetName)
        {
            ...
            position.X = (Global.width / 2); // Set the start X position of the player
            position.Y = (Global.height / 2); // Set the start Y position of the player
            ...
            // Load all Animations
            LoadAnimation("PlayerSprites/Melodiva/idle/BirdIdle@4x1", "idle", true, 0.2f);
            LoadAnimation("PlayerSprites/Melodiva/walk/BirdWalk@4x1", "walk", true, 0.2f);
            LoadAnimation("PlayerSprites/Melodiva/attack/BirdAttack@4x1", "attack", true, 0.2f);
            LoadAnimation("PlayerSprites/Melodiva/hurt/BirdHurt@1x1", "hurt", true, 1f);
        }
        ...
        // Method to handle the input of PlayerTwo specific
        public override void HandleInput(InputHelper inputHelper)
        {
            ...
            if (!IsDead) // If the player is not dead
            {
                ...
                //Move Right
                else if (inputHelper.IsKeyDown(Keys.L))
                {
                ...
                else if (!inputHelper.IsKeyDown(Keys.P))
                {
                ...
                //Move down
                else if (inputHelper.IsKeyDown(Keys.K))
                {
                    velocity.Y = ySpeed;
                }
                ...
                {
                    //Attack Left
                    if (inputHelper.IsKeyDown(Keys.P) && (direction == "Left") && attackcooldown == 0 && !attacking)
                    {
                        ...
                        if (animations["attack"].SheetIndex == 2)//checks if it's at the "impact frame"
                        {
                            ...
                            playingState.DeleteFirstHitbox(2); // Remove the first hitbox in a list from hitboxes from player Two
                            playingState.AddPlayerAttackHitboxesLeft(2, this.Position); // Add a new attack hitbox for player Two
                    ...
                    {
                        playingState.DeleteFirstHitbox(2); // Remove the first hitbox in a list from hitboxes from player One
                    }

                    //Attack Right
                    if (inputHelper.IsKeyDown(Keys.P) && direction == "Right" && attackcooldown == 0 && !attacking)
                    {
                        attacking = true;//lets the game know the player is doing it's attacking animation to avoid spam
                        PlayAnimation("attack");//plays the animation
                        if (animations["attack"].SheetIndex == 2)//checks if it's at the "impact frame"
                        {
                            attackcooldown = 30; // The cooldown of the Attack in frames
                            playingState.DeleteFirstHitbox(2); // Remove the first hitbox in a list from hitboxes from player Two
                            playingState.AddPlayerAttackHitboxesRight(2, this.Position); // Add a new attack hitbox for player Two
                            MainGame.AssetManager.PlaySound("Sounds/soundEffects/attack"); // Play the attack soundEffect
                            staminaBar.UseStamina(20); // Use a certain amount of stamina when the player uses an attack
                        }
                        else
        ...
        public override void TakeDamage(int damage)
        {
            ...
            MainGame.AssetManager.PlaySound("Sounds/soundEffects/femalehurt"); // Play the femaleHurt soundEffect 
        }
    }
}